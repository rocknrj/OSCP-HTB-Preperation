...
cat <file_name> | jq .
```
	- this file shows different endpoints which we can try to access with our SSRF. We end up finding another file in this endpoint:
		```bash
...
```
	- we see the input argument to pass some command.
